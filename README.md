...
AI 會看你讀咒的空檔丟 `silence` 打斷你、血少時補血開盾,每施放一次咒就升一級(最高 9),
讀咒速度越來越快、也會解鎖越強的咒。你對它丟 `frost` 會凍住它的讀條,`scramble` 會讓它讀得變慢。

## 咒語一覽

六招,血量 100。越強的咒要打越多段咒文,讀得越久也越容易被 `silence` 打斷。

| 咒語 | 咒文字數 | 效果 |
|------|---------|------|
| `spark` 火花 | 2 | 12 傷 |
| `frost` 冰霜 | 2 | 8 傷,並凍結對手 2.2 秒(那段時間他不能打字) |
| `tornado` 龍捲風 | 3 | 15 傷,並打亂對手 3 秒內的咒文順序 |
| `tsunami` 海嘯 | 5 | 40 傷 |
| `heal` 治療 | 3 | +26 血 |
| `silence` 沉默 | 2 | 中斷對手正在讀的咒 |
